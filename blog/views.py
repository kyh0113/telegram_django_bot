import json
import telegram
from test_telegram_bot import bot
from django.views.decorators.csrf import csrf_exempt
from django.http import HttpResponse
from django.shortcuts import render
from .text import start, regist, pause, image, usage
    ...
    if ((is_json_key_present(telegram_update['message'], 'document')) and (telegram_update['message']['document']['mime_type'] == "image/jpeg" \
        ...
        try: # 초기 등록 이미지에 대한 예외처리
            ...
            try: # 연속 이미지 등록에 대한 예외 처리 
                ...
                print('Already regist Image : 이미지 등록')
                text = "진행 중인 이미지에 대한 1번째 질문을 입력해주세요. (연속 등록 시, 선 등록 이미지 기준으로 질문을 입력해주세요.)"
...
완료 : {2}  중지 : {3} '''.format(userName, count_total_image, count_complete_image, count_pause_image)
                    bot.send_message(chat_id=chatID, text=text)
